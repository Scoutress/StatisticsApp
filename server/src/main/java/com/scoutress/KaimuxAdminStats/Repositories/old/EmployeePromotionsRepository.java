...
public interface EmployeePromotionsRepository extends JpaRepository<EmployeePromotions, Long> {
    ...
    @Query("SELECT new com.scoutress.KaimuxAdminStats.Entity.Employees.EmployeePromotionsPlus(e.id, e.username, e.level, ep.toSupport, ep.toChatmod, ep.toOverseer, ep.toManager) "
            +
    ...
    @Query("SELECT e.toSupport FROM EmployeePromotions e WHERE e.employeeId = :employeeId")
    Optional<LocalDate> findToSupportPromotionDateByEmployeeId(@Param("employeeId") Integer employeeId);
