...
public class RecommendationServiceImpl implements RecommendationService {
  ...
  public void evaluateEmployees() {
    ...
    for (Employee employee : employees) {
      try {
        double playtimeHours = dailyPlaytimeRepository.sumPlaytimeByEmployeeAndDateRange(employee.getId(),
            currentDate.minusYears(1), currentDate);
        System.out.println("Employee ID: " + employee.getId() + " Playtime Hours: " + playtimeHours);
  ...
  public boolean checkPlaytime(int employeeId, LocalDate startDate, LocalDate endDate) {
    double playtimeHours = dailyPlaytimeRepository.sumPlaytimeByEmployeeAndDateRange(employeeId, startDate, endDate);
